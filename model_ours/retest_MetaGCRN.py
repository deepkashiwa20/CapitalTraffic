...
def getModel(mode):
    model = MetaGCRN(num_nodes=num_variable, input_dim=opt.channelin, output_dim=opt.channelout, horizon=opt.seq_len, 
                        rnn_units=opt.hiddenunits, num_layers=opt.num_layers, decoder_type=opt.decoder, go_type=opt.go).to(device)
    if mode == 'train':
...
def testModel(name, mode, XS, YS, YCov, Mask=None):
    def testScore(YS, YS_pred, message):
        ...
        print("%s, %s, Torch MSE, %.6f" % (name, mode, loss))
    ...
    loss, YS_pred = evaluateModel(model, test_iter, YCov is not None)
...
parser.add_argument('--model', type=str, default='MetaGCRN', help='which model to use')
parser.add_argument('--gpu', type=int, default=3, help='which gpu to use')
opt = parser.parse_args()
# optimal1: --ycov=history --go=random 
# optimal2: --ycov=time --go=last
...
# traintest cmd: python traintest_MetaGCRN.py --gpu=2 --ycov=history --go=random
# retest cmd: python retest_MetaGCRN.py 
# The default setting is same with traintest cmd, so we can just run python retest_MetaGCRN.py.
path = f'./save/tokyo202112_MetaGCRN_c1to1_20220204042624_history'
