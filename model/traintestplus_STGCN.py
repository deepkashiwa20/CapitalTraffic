...
def refineXSYS(XS, YS):
    ...
    XS, YS = XS.transpose(0, 3, 1, 2), YS.transpose(0, 3, 1, 2)
    return XS, YS

def refineYS_ex(YS):
    YS = YS[:, :, :, opt.channelout:]
        ...
        if opt.channelin > 1:
            for x, y, ex in data_iter:
                ...
                for i in range(opt.seq_len):
                    ...
                    XS_pred_multi_batch.append(torch.cat((yhat, ex[:, :, i:i+1, :]), axis=1))
...
def main():
    ...
    train_data = [get_data(config[month]['trafficplus_path'], N_link, subroad_path, feature_list) for month in train_month]
    test_data = [get_data(config[month]['trafficplus_path'], N_link, subroad_path, feature_list) for month in test_month]
