...
def HistoricalAverage(data_list, time_list, YS_index):
    data = np.vstack(data_list)[..., 0:1]
    isholiday = np.vstack(time_list)[..., -1]
    print(data.shape, isholiday.shape)
    HISTORYDAY, DAYTIMESTEP = 7, 144
    XS_Week, YS = [], []
    for i in range(YS_index.shape[0]):
        if YS_index[i, 0] < HISTORYDAY*DAYTIMESTEP:
            continue
        Week, Y = [], []
        for j in range(YS_index.shape[1]):
            index = YS_index[i, j]
            mask = [k for k in range(index-HISTORYDAY*DAYTIMESTEP,index,DAYTIMESTEP) if isholiday[k]==isholiday[index]]
            Week.append(np.mean(data[mask, :], axis=0))
            Y.append(data[index, :])
        XS_Week.append(Week)
        YS.append(Y)
...
def testModel(name, mode, data, time, YS_index):
    logger.info(opt.city, opt.month, 'TIMESTEP_IN, TIMESTEP_OUT', opt.his_len, opt.seq_len)
    YS, YS_pred = HistoricalAverage(data, time, YS_index)
...
def main():
    ...
    testModel(model_name, 'test', test_data, test_onehottime, testYS_idx)
