...
def getModel():
    # original
    model = Transformer(in_channels=opt.channelin, out_channels=opt.channelout, embed_size=64, pe_length=opt.his_len, num_layers=1, 
                        timestep_in=opt.his_len, timestep_out=opt.seq_len, heads=8, forward_expansion=32, gpu=device, dropout=0).to(device)
    return model
