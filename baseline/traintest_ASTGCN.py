...
def getXSYS_WeekDayHour(data_list, his_len, seq_len):
    DAY_TIMESTEP = 144
    start_index = DAY_TIMESTEP*7*WEEK
    XS, YS = [], []
    for data in data_list:
        for i in range(data.shape[0] - seq_len - start_index + 1):
...
def getModel():
    A = get_adj(adj_path, subroad_path)
    W = weight_matrix(A, sigma2=1.0)
    L = scaled_laplacian(W)
    cheb_polynomials = [torch.from_numpy(i).type(torch.FloatTensor).to(device) for i in cheb_poly(L, Ks=3)]
    model = ASTGCN(device, cheb_polynomials, num_for_predict=opt.seq_len, len_input=opt.his_len*(WEEK+DAY+HOUR), num_of_vertices=num_variable, in_channels=opt.channelin, nb_block=1, K=3, nb_chev_filter=32, nb_time_filter=32, time_strides=2).to(device)
...
def main():
    ...
    trainXS, trainYS = getXSYS_WeekDayHour(train_data, opt.his_len, opt.seq_len)
    ...
    testXS, testYS = getXSYS_WeekDayHour(test_data, opt.his_len, opt.seq_len)
