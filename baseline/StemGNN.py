...
class StockBlockLayer(nn.Module):
    ...
    def spe_seq_cell(self, input):
        ...
        ffted = torch.fft.fft(input, norm='backward') # https://pytorch.org/docs/1.7.1/generated/torch.rfft.html?highlight=rfft#torch.rfft
        ...
        time_step_as_inner = torch.view_as_complex(torch.cat([real.unsqueeze(-1), img.unsqueeze(-1)], dim=-1))
        iffted = torch.fft.ifft(time_step_as_inner, norm='forward') # https://pytorch.org/docs/1.7.1/generated/torch.irfft.html?highlight=rfft#torch.irfft
        return iffted.real
...
def main():
    ...
    model = StemGNN(units=N_NODE, stack_cnt=2, time_step=TIMESTEP_IN, multi_layer=5, horizon=TIMESTEP_OUT, device=device)
