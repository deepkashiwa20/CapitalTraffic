...
def getModel():
    model = StemGNN(units=num_variable, stack_cnt=2, time_step=opt.his_len, multi_layer=5, horizon=opt.seq_len, device=device).to(device)
    summary(model, (opt.his_len, num_variable), device=device)
    return model
